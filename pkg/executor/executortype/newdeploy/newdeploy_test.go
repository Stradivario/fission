...
func TestWaitForBuild(t *testing.T) {
	...
	// A package that genuinely does not exist (deleted, or a Function
	// referencing one that was never created) must fail fast — not block for
	// the full defaultBuildWaitTimeout, which would otherwise hang a cold-start
	// invocation for up to 10 minutes on a plain NotFound.
	t.Run("missing package fails fast instead of waiting the full window", func(t *testing.T) {
		t.Setenv("NEWDEPLOY_BUILD_WAIT_TIMEOUT", "600")
		d := newDeploy() // no Package seeded: every Get returns NotFound
		start := time.Now()
		err := d.waitForBuild(t.Context(), fnFor("pkg", "default"))
		require.Error(t, err)
		assert.Contains(t, err.Error(), "not found")
		assert.Less(t, time.Since(start), 30*time.Second,
			"a genuinely missing package must fail within the short NotFound grace period, not the full build-wait window")
	})
}
