...
func (deploy *NewDeploy) updateFuncDeployment(ctx context.Context, fn *fv1.Function, env *fv1.Environment) error {
	...
	if deploy.deployEventHook.Enabled() {
		...
		// A scale-to-zero function (MinScale 0, the normal convention for
		// dev/test lambdas — see docs/federation-lambdas.md) legitimately has
		// 0 replicas between invocations; updateFuncDeployment does not scale
		// it up (only createOrGetDeployment does, and only at create time).
		// Flooring to 1 here — the original bug — waited for a replica that
		// was never going to appear on its own, so every scale-to-zero
		// function's update falsely fired function.deploy.failed even though
		// the spec patch fully succeeded and is correctly served on the next
		// on-demand specialize. Waiting for 0 succeeds immediately (0 >= 0)
		// and correctly fires function.deploy.ready right away: the new spec
		// IS live, there is just no pod expected to exist yet.
		...
		if werr != nil {
			...
			// attempt — a real incident: a stream of function.deploy.failed
			// events every few seconds, indefinitely, observed after this
			// hook was first enabled against a slow deployment. Same
			// "never permanently block, never keep re-litigating" posture as
			// waitForBuild's own timeout fallback above.
			//
