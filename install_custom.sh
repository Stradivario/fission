...
for i in {1..10}; do
    helm repo update
    if helm search repo fission-custom -l | grep -q "1.24.0-watch-all-namespaces"; then
        echo "Found version 1.24.0-watch-all-namespaces!"
        break
    fi
...
echo "Installing Fission 1.24.0-watch-all-namespaces..."
helm install fission fission-custom/fission-all \
  --namespace fission \
  --create-namespace \
  --version 1.24.0-watch-all-namespaces
